    ...
    return (
        <div className='space-y-4 rounded-md mt-2'>
            <div className='bg-gray-100 flex gap-7 items-center p-2 rounded-md'>
                <img src={product.thumbnail} alt={product.title}  className='md:h-60 md:w-60 h-25 w-25 rounded-md cursor-pointer' 
                ...
                <div className='space-y-2'>
                    ...
                    <p className='text-xs'>FREE delivery <span className='font-semibold'>Fri, 18 Apr</span> <br />
                    Or fastest delivery <span className='font-semibold'>Tomorrow, 17 Apr</span></p>
                    <button onClick={()=>addToCart(product)} className='cursor-pointer bg-red-500 text-white px-3 py-1 rounded-md'>Add to Cart</button>
                </div>
            </div>
        </div>
    )
